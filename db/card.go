...
func CreateCards(tableItem string, group string, cards []models.Card, v int) (re []models.CardMeta, newV int, err error) {
	re = make([]models.CardMeta, len(cards))
	newV = 0
	...
	query := fmt.Sprintf(`
		INSERT INTO %[1]ss (id, %[2]s_v, %[2]s_sync_v, %[2]s_data)
		VALUES (?, ?, ?, ?)
		ON CONFLICT(id) DO UPDATE SET
		%[2]s_v = excluded.%[2]s_v,
		%[2]s_data = excluded.%[2]s_data
		RETURNING %[2]s_sync_v;
	`, tableItem, group)
	// fmt.Println(query)
	stmt, err := tx.Prepare(query)
	if err != nil {
	...
	tn := fmt.Sprintf("%s_%ss", tableItem, group)
	_, err = tx.Exec("UPDATE versions SET val = ? WHERE id = ?", v, tn)
	if err != nil {
		return
	}
